...
# 9. Puesta en marcha con Docker

El ecosistema de microservicios también puede ejecutarse utilizando **Docker** y **Docker Compose**, lo que permite levantar todos los componentes necesarios del sistema en contenedores independientes.
...
Para levantar todo el ecosistema (usando el archivo `docker-compose.yml` provisto en el empaquetado):

```bash
docker compose up -d
```

El orden lógico de arranque está configurado en `docker-compose.yml` para asegurar un inicio seguro:
1. **MySQL** inicia primero.
2. **Eureka Server** inicia en paralelo.
3. **Microservicios** inician únicamente cuando la base de datos está saludable ("healthy").
4. **API Gateway** inicia y se registra en Eureka.

## Comandos Útiles de Docker
*   **Detener el sistema**: `docker compose down` (Detiene la ejecución sin borrar los datos guardados en la base de datos).
*   **Ver logs en tiempo real**: `docker compose logs -f`
*   **Destruir el volumen de datos**: `docker compose down -v` *(Advertencia: Esto borrará la información de la base de datos de forma permanente).*
